...
class ChatController: UIViewController,UITextViewDelegate{
  ...
  //  var m_CommentArray = [CommetList]
    ...
    func SetUpList()
    {
        ...
        tblCommentList.register(ImageChatcell.self, forCellReuseIdentifier: cellIdentifierimage)
        tblCommentList.register(tableCell.self, forCellReuseIdentifier: cellIdentifiertable)
        tblCommentList.register(ChatRightCell.self, forCellReuseIdentifier: cellIdentifierright)
        ...
        tblCommentList.keyboardDismissMode = .onDrag
  ...
  //      tblCommentList.backgroundColor = UIColor.orange
    ...
    @objc func messageSendAction(sender:UIButton!) {
        if  tvTextInPutContainer.text.trimmingCharacters(in: .whitespacesAndNewlines).count>0{
            ...
            let chat = ChatModel.init(chatMessage: tvTextInPutContainer.text.trimmingCharacters(in: .whitespacesAndNewlines), userName: UserDetail.Instance.name!, IsSender: false, date:ShareData.sharedInstance.GetCurrentDateAndTime(),imageUrl: "" , mType: MessageType.text,mtextArray: [],mHeadersText: "")
            AppConstant.sharedInstance.chatItem.append(chat)
            UIView.performWithoutAnimation {
                self.dataSource?.chatItem =   AppConstant.sharedInstance.chatItem
                self.dataDelegate?.chatItem = AppConstant.sharedInstance.chatItem
                let index = IndexPath(item: AppConstant.sharedInstance.chatItem.count-1, section: 0)
                self.tblCommentList.insertRows(at: [index], with: UITableViewRowAnimation.fade)
                self.tblCommentList.scrollToRow(at: index, at: UITableViewScrollPosition.bottom, animated: false)
            }
            ChatResponse(chatText: tvTextInPutContainer.text.trimmingCharacters(in: .whitespacesAndNewlines))
        ...
        request?.setMappedCompletionBlockSuccess({ (request, response) in
            ...
            if  action  != "input.unknown" {
                ...
                UIView.performWithoutAnimation {
                    ...
                    self.dataDelegate?.chatItem = AppConstant.sharedInstance.chatItem
                    let index = IndexPath(item: AppConstant.sharedInstance.chatItem.count-1, section: 0)
                    self.tblCommentList.insertRows(at: [index], with: UITableViewRowAnimation.automatic)
                    self.tblCommentList.scrollToRow(at: index, at: UITableViewScrollPosition.bottom, animated: false)
                }
